...
class MaestroDataset(object):
   ...
   def __getitem__(self, meta):
        ...
        data_dict['segment_idx'] = start_time

        # relative time inside index
        note_events_shifted = []
        for i, event in enumerate(note_events):
            if event['onset_time'] >= start_time:
                note_events_shifted.append({
                    'onset_time': event['onset_time'] - start_time,
                    'offset_time': event['offset_time'] - start_time,
        ...
        for note in note_events_shifted:
            copy_of_note = note.copy()
            note.pop("offset_time", None)
            note['time'] = note['onset_time']
            del note['onset_time']
            copy_of_note.pop("onset_time", None)
            copy_of_note['time'] = copy_of_note['offset_time']
            copy_of_note['velocity'] = 0
            del copy_of_note['offset_time']
            new_note_events.append(note)
            new_note_events.append(copy_of_note)

        new_note_events = sorted(new_note_events, key=lambda k: k['time'])

        data_dict['note_events'] = new_note_events
        data_dict['note_events_ids'] = []

        data_dict['note_events_ids'].append(self.vocab_token2idx['[BOS]'])

        for event in data_dict['note_events']:
            # get ids
            id_midi_note_event = self.vocab_token2idx['[midi_note ' + str(event['midi_note']) + ']']
            id_velocity_event = self.vocab_token2idx['[velocity ' + str(event['velocity']) + ']']

            if round(event['time'], 2) == config.segment_seconds:
                id_time_event = self.vocab_token2idx['[time ' + str(round(event['time'] - 0.01, 2)) + ']']
            else:
                id_time_event = self.vocab_token2idx['[time ' + str(round(event['time'], 2)) + ']']

            # append ids
            data_dict['note_events_ids'].append(id_midi_note_event)
            data_dict['note_events_ids'].append(id_velocity_event)
            data_dict['note_events_ids'].append(id_time_event)

        data_dict['note_events_ids'].append(self.vocab_token2idx['[EOS]'])

        # print("===============Target len==============")
        # print(len(data_dict['note_events_ids']))
        # print("=======================================")

        if len(data_dict['note_events_ids']) < self.output_size:
            padding_size = self.output_size - len(data_dict['note_events_ids'])
            padding = padding_size * [self.vocab_token2idx['[PAD]']]
            data_dict['note_events_ids'] += padding

        assert  len(data_dict['note_events_ids']) == self.output_size
   ...
   def create_vocab(self):
    ...
    # insert 128 note events
    for note_value in range(1,129):
        vocab_idx2token[note_value+2] = '[midi_note ' + str(note_value) + ']'
        vocab_token2idx['[midi_note ' + str(note_value) + ']'] = note_value+2

        # insert 128 velocity events
    for vel_value in range(1,129):
        vocab_idx2token[vel_value+2+128] = '[velocity ' + str(vel_value - 1) + ']'
        vocab_token2idx['[velocity ' + str(vel_value - 1) + ']'] = vel_value+2+128

    # insert time events
    # 5 second = 5000 is 500 times 10ms
    for time_value in range(0, int(config.segment_seconds*100)):
        vocab_idx2token[time_value+2+128+1+128] = '[time ' + str(time_value / 100) + ']'
        vocab_token2idx['[time ' + str(time_value / 100) + ']'] = time_value + 2 + 128 + 1 + 128

    self.vocab_idx2token = vocab_idx2token
    self.vocab_token2idx = vocab_token2idx
    self.vocab_size = len(self.vocab_idx2token)
